...
//   FRONTEND_ORIGINS    — список разрешённых Origin для CORS через запятую
// Опциональные:
//   PORT                — порт (по умолчанию 3000)
//   SKIP_TWA_VERIFY     — 1 = отключить проверку подписи initData (только для диагностики)
// ───────────────────────────────────────────────────────────
...
app.use(
  cors({
    origin: (origin, cb) => {
      if (!origin) return cb(null, true) // сервер-сервер/curl
...
function verifyTgInitData(initData, botToken) {
  // initData — строка querystring (ключ=значение&…)
...
// Быстрые пинги с фронта (удобно видеть, что фронт жив)
...
app.post('/api/twa/auth', async (req, res) => {
  try {
    ...
    // здесь можно сохранить/обновить пользователя в БД и т.п.
...
app.post('/api/logs', (req, res) => {
  try {
    ...
    // здесь можно писать в БД (Supabase) или файлы
...
// лог на любой заход к вебхуку (помогает отлавливать 404 у Телеги)
